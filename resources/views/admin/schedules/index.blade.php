...
@endsection
@section('subtitle')
    List Jadwal Preventive Maintenance
@endsection
@section('menuUtama')
    Menu Utama
@endsection
...
@push('styles')
    <style>
        ...
        .pm-calendar td.today {
            background: #f0f3ff;
            border-color: #6366f1;
        }

        .pm-calendar td.other-month {
            background: #fafafa;
            opacity: .6;
        }
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="{{ $isToday ? 'today' : '' }}">
                                        <span
                                            class="cal-day-num {{ $isToday ? 'today-num' : '' }}">{{ $dayNum }}</span>
                                        <div style="margin-top: 18px;">
                                            ...
                                            @if (count($events) === 0 && $isToday)
                                                <div class="text-center mt-2">
                                                    <i class="bi bi-check2 text-success opacity-50"
                            ...
                            <form method="GET" class="d-flex flex-wrap gap-2 align-items-center">
                                ...
                                <select name="filter_group" class="form-select form-select-sm w-auto"
                                    ...
                                    @foreach ($etmGroups as $g)
                                        <option value="{{ $g }}" @selected($filterGroup === $g)>
                                            {{ $g }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($filterStatus || $filterCycle || $filterGroup)
                                    <a href="{{ route('admin.schedules.index') }}" class="btn btn-sm btn-light-danger">
                                        ...
                                        {{ $s->equipment_status === 'active' ? 'bg-light-success' : 'bg-light-warning' }}">
                                            ...
                                            {{ $s->equipment_status === 'active' ? 'text-success' : 'text-warning' }}"></i>
                                                ...
                                                <td class="text-muted fs-7">
                                                    {{ $s->last_maintenance ? \Carbon\Carbon::parse($s->last_maintenance)->format('d M Y') : '—' }}
                                                </td>
                                                <td>
                                                    <div
                                                        class="fw-semibold {{ $isLate ? 'text-danger' : 'text-gray-800' }}">
