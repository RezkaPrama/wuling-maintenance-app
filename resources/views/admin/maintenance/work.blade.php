...
.work-sticky-bar {
    ...
    background: #fff; border-bottom: 2px solid #eff2f5;
    padding: 12px 24px; margin: -24px -24px 24px -24px;
...
/* ── Check item card ── */
.check-item-card {
    border: 2px solid #eff2f5; border-radius: 10px;
    margin-bottom: 12px; transition: all .2s ease;
    background: #fff;
}
.check-item-card:hover { border-color: #d1d5db; box-shadow: 0 2px 8px rgba(0,0,0,.08); }
.check-item-card.status-ok      { border-left: 4px solid #50cd89; }
.check-item-card.status-ng      { border-left: 4px solid #f1416c; }
.check-item-card.status-na      { border-left: 4px solid #7e8299; }
.check-item-card.status-pending { border-left: 4px solid #e4e6ef; }

/* ── Status buttons ── */
.status-btn-group .btn { min-width: 60px; font-size: .78rem; font-weight: 600; border-radius: 6px; }
.status-btn-group .btn.active-ok  { background: #50cd89; color: #fff; border-color: #50cd89; }
.status-btn-group .btn.active-ng  { background: #f1416c; color: #fff; border-color: #f1416c; }
.status-btn-group .btn.active-na  { background: #7e8299; color: #fff; border-color: #7e8299; }
...
/* ── Photo thumbnails ── */
.photo-thumb { width: 64px; height: 64px; object-fit: cover;
    border-radius: 6px; cursor: pointer; border: 2px solid #eff2f5;
    transition: border-color .2s; }
.photo-thumb:hover { border-color: #6366f1; }

/* ── Progress bar ── */
.progress-work { height: 8px; border-radius: 999px; overflow: hidden; background: #f5f8fa; }
.progress-work .progress-bar { border-radius: 999px; transition: width .5s ease; }

/* ── Saving indicator ── */
.save-indicator { font-size: .75rem; }
.save-indicator.saving  { color: #f6a723; }
.save-indicator.saved   { color: #50cd89; }
.save-indicator.error   { color: #f1416c; }

/* ── Item number badge ── */
.item-num-badge {
    width: 28px; height: 28px; border-radius: 50%;
    background: #f5f8fa; display: flex; align-items: center; justify-content: center;
    font-size: .75rem; font-weight: 700; color: #5e6278; flex-shrink: 0;
}
</style>
@endpush
...
<div class="work-sticky-bar" id="stickyBar">
    ...
    <div class="d-flex gap-2">
        <a href="{{ route('admin.maintenance.record.show', $record->id) }}"
        ...
        <div class="card card-flush border-0 shadow-sm h-100">
            <div class="card-body py-4 px-5 d-flex flex-column justify-content-center">
                <div class="row g-2 text-center">
                    ...
                    <div class="col-4">
                        <div class="fs-3 fw-bold text-muted" id="statPending">{{ $progress['pending'] }}</div>
...
<div class="mb-6">
    ...
    <div class="check-item-card status-{{ $item->status }}"
         id="card-{{ $item->id }}" data-item-id="{{ $item->id }}">
        <div class="p-4">
            {{-- Item Header --}}
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex gap-1 mt-1 flex-wrap">
                        @foreach($item->pm_types as $pt)
                            <span class="badge badge-light-secondary fs-8">{{ $pt }}</span>
                        @endforeach
                    </div>
                    @endif
                </div>
            ...
            <div class="d-flex align-items-center gap-3 flex-wrap">
                ...
                <div class="form-check form-switch mb-0">
                    ...
                    <label class="form-check-label fs-8 text-muted" for="req-{{ $item->id }}">
                        Perlu tindakan
                    </label>
                </div>
            ...
            <div id="actionDetail-{{ $item->id }}"
                ...
                <div class="alert alert-light-warning border border-warning border-dashed py-3 mb-0">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-footer border-0">
                ...
                <form method="POST" action="{{ route('admin.maintenance.record.complete', $record->id) }}">
                    @csrf
...
function saveRemarks(itemId, value) {
    ...
    saveTimer['r' + itemId] = setTimeout(() => saveItem(itemId, { remarks: value }), 800);
}
...
function toggleAction(itemId, checked) {
    ...
    saveItem(itemId, { requires_action: checked });
}
