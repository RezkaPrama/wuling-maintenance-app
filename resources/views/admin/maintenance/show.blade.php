...
@section('subtitle')
    Tambah Jadwal Preventive Maintenance Record
@endsection
@section('menuUtama')
    Menu Utama
@endsection
                ...
                <div class="d-flex align-items-start justify-content-between mb-6 flex-wrap gap-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        @if ($record->status === 'in_progress')
                            <a href="{{ route('admin.records.work', $record->id) }}" class="btn btn-warning">
                        ...
                        @if (in_array($record->status, ['completed', 'validated']))
                            <a href="{{ route('admin.records.export', $record->id) }}" class="btn btn-sm btn-light-success">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-6">
                                                ...
                                                @endphp
                                                <span
                                                    class="badge {{ $st['class'] }} fw-semibold">{{ $st['label'] }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-6">
                                                ...
                                                @if ($record->start_time && $record->end_time)
                                                    @php
                                                        $dur = \Carbon\Carbon::parse(
                                                            $record->maintenance_date . ' ' . $record->start_time,
                                                        )->diffInMinutes(
                                                            \Carbon\Carbon::parse(
                                                                $record->maintenance_date . ' ' . $record->end_time,
                                                            ),
                                                        );
                                                    @endphp
                                ...
                                <div class="row align-items-center">
                                    <div class="col-auto">
                                        <div class="position-relative d-inline-flex align-items-center justify-content-center"
                                            ...
                                            <svg viewBox="0 0 36 36" class="position-absolute"
                                                style="width:80px;height:80px;transform:rotate(-90deg)">
                                                <circle cx="18" cy="18" r="15.9" fill="none"
                                                    stroke="#eff2f5" stroke-width="3" />
                                                <circle cx="18" cy="18" r="15.9" fill="none"
                                                    stroke="#50cd89" stroke-width="3"
                            ...
                            <div class="card-body pt-0">
                                ...
                                @foreach ($items as $item)
                                    @if ($item->sub_equipment !== $lastSubEq)
                                        ...
                                        @endif
                                        <div
                                            class="fw-bold text-muted fs-8 text-uppercase letter-spacing-1 mb-2 mt-4
                            {{ $loop->first ? 'mt-0' : '' }}">
                                    ...
                                    <div class="d-flex align-items-start gap-3 p-3 rounded mb-2 item-row-{{ $item->status }}"
                                        ...
                                        <div class="text-end">
                                            @php
                                                ...
                                                $ist = $itemStatusMap[$item->status] ?? [
                                                    'class' => 'badge-light',
                                                    'label' => $item->status,
                                                ];
                                            @endphp
                                            <span
                                                class="badge {{ $ist['class'] }} fw-semibold px-3">{{ $ist['label'] }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="card-body pt-3">
                                @php
                                    $steps = [
                                        ...
                                        [
                                            ...
                                            'done' => in_array($record->status, ['completed', 'validated', 'rejected']),
                                            'name' => null,
                                            'color' => in_array($record->status, ['completed', 'validated', 'rejected'])
                                                ? 'success'
                                                : 'secondary',
                                            'icon' => 'bi-clipboard-check',
                                        ],
                                        [
                                            'label' => 'Validasi Checker',
                                            'done' => in_array($record->status, ['validated', 'rejected']),
                                            'name' => $record->checker_name,
                                            'color' =>
                                                $record->status === 'rejected'
                                                    ? 'danger'
                                                    : (in_array($record->status, ['validated'])
                                                        ? 'success'
                                                        : 'secondary'),
                                ...
                                @foreach ($steps as $step)
                                    <div class="timeline-step">
                                        <div
                                            class="timeline-dot bg-{{ $step['done'] ? $step['color'] : 'light' }}
                        {{ $step['done'] ? 'text-white' : 'text-muted' }}">
                                            <i class="bi {{ $step['icon'] }}" style="font-size:.55rem"></i>
                                        </div>
                                        <div
                                            class="fw-semibold fs-7 {{ $step['done'] ? 'text-gray-800' : 'text-muted' }}">
                                            {{ $step['label'] }}
                                        </div>
                                        @if ($step['name'])
                        ...
                        @if (in_array($record->status, ['completed', 'validated']))
                            <div class="card card-flush border-0 shadow-sm" id="validate">
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="{{ route('admin.records.validasi', $record->id) }}"
                                        id="form-validasi">
                                        @csrf
                                        <div class="mb-4">
                                            <label class="form-label fw-semibold">Catatan Validasi</label>
                                            <textarea name="notes" rows="3" class="form-control" placeholder="Tambahkan catatan validasi (opsional)..."></textarea>
                                        </div>
                                        <div class="d-flex gap-2">
                                            {{-- Tombol trigger SweetAlert (type="button") --}}
                                            <button type="button" id="btn-approve" class="btn btn-success flex-grow-1">
                                            ...
                                            {{-- Submit asli disembunyikan, dipanggil setelah konfirmasi --}}
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('btn-approve').addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: 'Record ini akan disetujui dan diteruskan ke tahap berikutnya.',
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#50cd89',
                    cancelButtonColor: '#b5b5c3',
                    confirmButtonText: '<i class="bi bi-check-circle me-1"></i> Ya, Approve',
                    cancelButtonText: 'Batal',
                    reverseButtons: true,
                }).then((result) => {
                    if (result.isConfirmed) {
                        document.getElementById('form-validasi').querySelector(
                            '[name="action"][value="validate"]').click();
                    }
                });
            });
            ...
            document.getElementById('btn-reject').addEventListener('click', function(e) {
                ...
                Swal.fire({
                    title: 'Tolak Record?',
                    text: 'Record akan dikembalikan ke teknisi untuk diperbaiki.',
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('form-validasi').querySelector(
                            '[name="action"][value="reject"]').click();
                    }
                });
            });
