                        ...
                        <div class="col-lg-5">
                            <div class="card card-flush border-0 shadow-sm h-100">
                                ...
                                <div class="card-body pt-2">
                                    ...
                                    @if ($schedule)
                                        <div
                                            class="alert alert-light-primary border border-primary border-dashed rounded mb-5">
                                            <div class="d-flex align-items-start gap-3">
                                                <i class="bi bi-info-circle text-primary fs-4 mt-1"></i>
                                                <div>
                                                    <div class="fw-bold text-gray-800">{{ $schedule->equipment_name }}</div>
                                                    <div class="text-muted fs-7">{{ $schedule->equipment_code }} ·
                                                        {{ $schedule->etm_group }}</div>
                                                    <div class="d-flex gap-2 mt-2">
                                                        <span
                                                            class="badge badge-light-primary">{{ $schedule->pm_cycle }}</span>
                                                        <span
                                                            class="badge {{ $schedule->status === 'overdue' ? 'badge-light-danger' : 'badge-light-warning' }}">
                                    ...
                                    @else
                                        ...
                                        <div class="mb-5">
                                            <label class="form-label required fw-semibold">Pilih Jadwal
                                                (Due/Overdue)</label>
                                            <select name="schedule_id" id="scheduleSelect"
                                                ...
                                                @foreach ($dueSchedules as $ds)
                                                    <option value="{{ $ds->id }}"
                                                        ...
                                                        data-cycle="{{ $ds->pm_cycle }}" data-status="{{ $ds->status }}"
                                                        ...
                                                        ({{ $ds->pm_cycle }} ·
                                                        {{ \Carbon\Carbon::parse($ds->next_maintenance)->format('d M Y') }})
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('schedule_id')
                            ...
                            <div class="card card-flush border-0 shadow-sm mb-5">
                                ...
                                <div class="card-body pt-2">
                                    <textarea name="notes" rows="4" class="form-control"
                                        placeholder="Catatan tambahan sebelum memulai PM (opsional)...">{{ old('notes') }}</textarea>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#createRecordForm').on('submit', function() {
                $('#submitBtn').html(
                        '<span class="spinner-border spinner-border-sm me-2"></span>Membuat Record...')
                    .prop('disabled', true);
            });
        });
    </script>
