...
function initializeTransactionChangeHandler() {
    $('#j_transaksi, #jenis_barang').change(function () {
        ...
        // ← TAMBAHKAN INI
        const konsinyasiTypes = ['KONSINYASI', 'RETUNABLE'];
        if (konsinyasiTypes.includes(jTransaksi)) {
            $('#label-jenis-transaksi').text(jTransaksi);
            $('#info-konsinyasi').show();
        } else {
            $('#info-konsinyasi').hide();
        }
        // ← SAMPAI SINI
...
function addProductToCart(product, stockInfo = null) {
    ...
    const existingItemIndex = cartState.items.findIndex(i => i.id_product === product.id);

    if (existingItemIndex !== -1) {
        const item = cartState.items[existingItemIndex];

        // Tambah berdasarkan mode
        if (cartState.scanMode === 'pack') {
            item.qty_dz  += 1;
        } else {
            item.qty_psg += 1;
        ...
        // Normalisasi: setiap 24 psg = 1 dz
        normalizeQty(item);
        recalcItem(item);
        ...
        showCompactNotification(
            `+1 ${cartState.scanMode === 'pack' ? 'DZ' : 'Psg'} ${product.name} ` +
            `(${item.qty_dz} DZ ${item.qty_psg} Psg)`
        );
    } else {
        const newItem = {
            ...
            quantity:     product.quantity,   // qty/pack (isi pack)
            price_pcs:    price,
            hpp_pcs:      hpp,
            qty_dz:  cartState.scanMode === 'pack'   ? 1 : 0,  // BARU
            qty_psg: cartState.scanMode === 'pasang' ? 1 : 0,  // BARU
            subtotal:     0,
            hpp_total:    0,
        };

        normalizeQty(newItem);
        recalcItem(newItem);
        cartState.items.push(newItem);
        addCartRowOptimized(newItem);

        playSuccessSound();
        let stockText = stockInfo
            ? ` (Stock: ${stockInfo.saldo_akhir_dz} DZ ${stockInfo.saldo_akhir_psg} Psg)`
            : '';
        showCompactNotification(`✓ ${product.name}${stockText}`);
    }
...
// BARU: Normalisasi 24 psg → 1 dz
function normalizeQty(item) {
    if (item.qty_psg >= PACK_TO_PSG) {
        const extra = Math.floor(item.qty_psg / PACK_TO_PSG);
        item.qty_dz  += extra;
        item.qty_psg -= extra * PACK_TO_PSG;
    }
    // Pastikan tidak negatif
    if (item.qty_psg < 0) item.qty_psg = 0;
    if (item.qty_dz  < 0) item.qty_dz  = 0;
}

// BARU: Hitung subtotal berdasarkan qty_dz + pecahan pasang
function recalcItem(item) {
    // Harga per pasang = harga pack / PACK_TO_PSG
    const pricePerPsg = item.price_pcs / PACK_TO_PSG;

    item.subtotal  = (item.qty_dz * item.price_pcs) + (item.qty_psg * pricePerPsg);
    item.hpp_total = (item.qty_dz * item.hpp_pcs)   + (item.qty_psg * (item.hpp_pcs / PACK_TO_PSG));

    // Total qty dalam satuan pack (untuk laporan)
    item.qty = item.qty_dz; // pack utuh (untuk backward compat)
}
...
function addCartRowOptimized(item) {
        ...
        <tr id="${rowId}">
            ...
            <td class="align-middle text-center">
                <div class="input-group input-group-sm justify-content-center">
                    <span class="input-group-text btn btn-sm btn-secondary qty-dz-decrease"
                          data-product-id="${item.id_product}">-</span>
                    <input type="number" class="form-control text-center product-qty-dz"
                           data-product-id="${item.id_product}"
                           value="${item.qty_dz}" min="0" style="width:55px">
                    <span class="input-group-text btn btn-sm btn-secondary qty-dz-increase"
                          data-product-id="${item.id_product}">+</span>
                </div>
                <small class="text-muted">DZ</small>
                ...
                <input type="hidden" name="products[${item.id_product}][qty]"          value="${item.qty_dz}">
                <input type="hidden" name="products[${item.id_product}][qty_psg]"      value="${item.qty_psg}">
            ...
            {{-- Kolom Pasang (PSG) — BARU --}}
            <td class="align-middle text-center">
                <div class="input-group input-group-sm justify-content-center">
                    <span class="input-group-text btn btn-sm btn-outline-teal qty-psg-decrease"
                          data-product-id="${item.id_product}">-</span>
                    <input type="number" class="form-control text-center product-qty-psg"
                           data-product-id="${item.id_product}"
                           value="${item.qty_psg}" min="0" max="23" style="width:55px">
                    <span class="input-group-text btn btn-sm btn-outline-teal qty-psg-increase"
                          data-product-id="${item.id_product}">+</span>
                </div>
                <small class="text-muted">Psg</small>
            </td>
    ...
    requestAnimationFrame(() => HighlightManager.highlight(rowId, true));
...
function updateCartRowOptimized(item, isNew = false) {
    ...
    setVal('.product-qty-dz',  item.qty_dz);
    setVal('.product-qty-psg', item.qty_psg);
    setText('.subtotal-value', formatMoney(item.subtotal));

    setVal(`input[name="products[${item.id_product}][qty]"]`,      item.qty_dz);
    setVal(`input[name="products[${item.id_product}][qty_psg]"]`,  item.qty_psg);
    setVal(`input[name="products[${item.id_product}][subtotal]"]`, item.subtotal);
    setVal(`input[name="products[${item.id_product}][hpp_total]"]`,item.hpp_total);

    requestAnimationFrame(() => HighlightManager.highlight(rowId, false));
}
...
function attachCartEvents(productId) {
    // Pack DZ controls
    $(`.qty-dz-increase[data-product-id="${productId}"]`).on('click', () => changeQtyDz(productId, +1));
    $(`.qty-dz-decrease[data-product-id="${productId}"]`).on('click', () => changeQtyDz(productId, -1));
    $(`.product-qty-dz[data-product-id="${productId}"]`).on('change', function () {
        const item = cartState.items.find(i => i.id_product === productId);
        if (item) {
            item.qty_dz = Math.max(0, parseInt($(this).val()) || 0);
            normalizeQty(item);
            recalcItem(item);
            updateCartRowOptimized(item);
            updateCartSummary();
        }
    });

    // Pasang PSG controls
    $(`.qty-psg-increase[data-product-id="${productId}"]`).on('click', () => changeQtyPsg(productId, +1));
    $(`.qty-psg-decrease[data-product-id="${productId}"]`).on('click', () => changeQtyPsg(productId, -1));
    $(`.product-qty-psg[data-product-id="${productId}"]`).on('change', function () {
        const item = cartState.items.find(i => i.id_product === productId);
        if (item) {
            item.qty_psg = Math.max(0, parseInt($(this).val()) || 0);
            normalizeQty(item);
            recalcItem(item);
            updateCartRowOptimized(item);
            updateCartSummary();
    ...
    // Remove
    $(`.remove-item[data-product-id="${productId}"]`).on('click', () => removeCartItem(productId));
}

function changeQtyDz(productId, delta) {
    const item = cartState.items.find(i => i.id_product === productId);
    if (!item) return;
    item.qty_dz = Math.max(0, item.qty_dz + delta);
    // Jika keduanya 0 setelah decrease, minimal 0
    if (item.qty_dz === 0 && item.qty_psg === 0 && delta < 0) return;
    recalcItem(item);
    updateCartRowOptimized(item);
    updateCartSummary();
}

function changeQtyPsg(productId, delta) {
    const item = cartState.items.find(i => i.id_product === productId);
    if (!item) return;
    const newPsg = item.qty_psg + delta;
    if (newPsg < 0 && item.qty_dz > 0) {
        item.qty_dz  -= 1;
        item.qty_psg  = PACK_TO_PSG - 1;
    } else {
        item.qty_psg = Math.max(0, newPsg);
    }
    normalizeQty(item);
    recalcItem(item);
    updateCartRowOptimized(item);
...
function updateCartSummary() {
    cartState.hpp        = cartState.items.reduce((s, i) => s + (parseFloat(i.hpp_total) || 0), 0);
    cartState.bruto      = cartState.items.reduce((s, i) => s + (parseFloat(i.subtotal)  || 0), 0);
    cartState.totalPack  = cartState.items.reduce((s, i) => s + (parseInt(i.qty_dz)      || 0), 0);
    cartState.totalPasang= cartState.items.reduce((s, i) => s + (parseInt(i.qty_psg)     || 0), 0);

    // Total pasang global → konversi ke pack juga
    const extraDz  = Math.floor(cartState.totalPasang / PACK_TO_PSG);
    const restPsg  = cartState.totalPasang % PACK_TO_PSG;
    const totalDz  = cartState.totalPack + extraDz;

    $("#total-pack").text(`${totalDz} DZ`);
    $("#total-pasang").text(`${restPsg} Psg`);
    $("#total_pack").val(totalDz);
    $("#total_pasang_input").val(restPsg);
...
function submitForm() {
    ...
    cartState.items.forEach((item, index) => {
        ...
        formData.append(`products[${index}][qty]`,          item.qty_dz);   // pack DZ
        formData.append(`products[${index}][qty_psg]`,      item.qty_psg);  // sisa pasang
