...
function addCartRowOptimized(item) {
    ...
    const newRow = `
        <tr id="${rowId}">
            ...
            <td class="align-middle text-end">${formatMoney(item.hpp_pcs)}</td>
...
function sortCartTableOptimized() {
    ...
    // Sort dengan item terbaru di atas (berdasarkan order penambahan)
    rows.sort((a, b) => {
        // Ambil index dari cartState.items
        const productIdA = a.id.replace('cart-item-', '');
        const productIdB = b.id.replace('cart-item-', '');
        
        const indexA = cartState.items.findIndex(item => item.id_product == productIdA);
        const indexB = cartState.items.findIndex(item => item.id_product == productIdB);
        
        // Item terakhir ditambahkan (index lebih besar) muncul di atas
...
function increaseItemQuantity(productId) {
    ...
    if (itemIndex !== -1) {
        ...
        item.hpp_subtotal = item.qty * item.hpp_pcs; // ✅ TAMBAHKAN
        updateCartRow(item);
        updateCartSummary();
    }
}
...
function submitForm() {
    const form = $("#receipt-manual-form");
    ...
    // ✅ OPTIMASI: Kirim products sebagai JSON, bukan individual inputs
    const formData = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        no_penerimaan: $('#no_penerimaan').val(),
        tgl_terima: $('input[name="tgl_terima"]').val(),
        penyerah: $('#penyerah').val(),
        id_warehouse_from: $('#id_warehouse_from').val(),
        note: $('#note').val(),
        
        // Kirim products sebagai array JSON
        products: cartState.items.map(item => ({
            ...
            price_pack: item.price_sell || 0,
            subtotal: item.subtotal || 0,
            hpp_pcs: item.hpp_pcs || 0,
            hpp_total: item.hpp_total || 0,
            hpp_subtotal: item.hpp_subtotal || 0
        })),
        
        // Summary amounts
        hpp_amount: cartState.hpp,
        bruto_amount: cartState.bruto,
        discount_percentage: parseFloat($('#discount_percentage').val()) || 0,
        discount_amount: cartState.discount1,
        discount_percentage2: parseFloat($('#discount_percentage2').val()) || 0,
        discount_amount2: cartState.discount2,
        discount_percentage3: parseFloat($('#discount_percentage3').val()) || 0,
        discount_amount3: cartState.discount3,
        total_discount_amount: cartState.discount1 + cartState.discount2 + cartState.discount3,
        netto_amount: cartState.netto,
        total_pack: cartState.totalPack
    };
            ...
            if (response.success) {
                Swal.fire({
                    ...
                    html: `
                        ...
                        <p class="mt-3"><strong>No. Penerimaan: ${response.no_penerimaan}</strong></p>
                    ...
                    if (result.isConfirmed) {
                        const printUrl = `/admin/inventory/receipt-manual/${response.receiving_id}/print`;
