...
function loadExistingShippingData() {
    ...
    $.ajax({
        url: `/api/v1/receipt-online/${shippingId}/details`,
        type: 'GET',
        headers: {
        ...
        success: function (response) {
            if (response.success) {
                ...
                $('#id_warehouse_to').val(shipping.id_warehouse_to).trigger('change');
...
function initializeFormValidation() {
    $("#receipt-online-form").on('submit', validateFormSubmission);

    const form = document.getElementById('receipt-online-form');
...
function submitForm() {
    ...
    const formData = new FormData($("#receipt-online-form")[0]);
    ...
    // Send the request
    $.ajax({
        url: $("#receipt-online-form").attr('action'),
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
    ...
    if (response.success) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '/admin/manufacture/receipt-online';
            }
        });
    } else {
