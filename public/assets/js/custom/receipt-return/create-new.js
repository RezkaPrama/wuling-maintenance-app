...
// ============ SOLUSI: Queue System untuk Handle Multiple Scans ============
let scanQueue = [];
...
function generateInvoiceNumber() {
    $.ajax({
        ...
        success: function (response) {
            cartState.invoiceNumber = response.no_retur_penerimaan;
            $('#no_retur_penerimaan').val(response.no_retur_penerimaan);
        },
...
function initializeFormValidation() {
    $("#receipt-return-form").on('submit', validateFormSubmission);

    const form = document.getElementById('receipt-return-form');
...
function addProductToCart(product, stockInfo = null) {
    // ✅ UBAH: Gunakan COGS untuk harga transaksi
    const price = parseFloat(product.cogs?.toString().replace(/[^\d.-]/g, '')) || 0;
    
    // ✅ UBAH: Gunakan HPP dari BOM (akan tersedia dari API yang sudah diupdate)
    const hpp = parseFloat(product.hpp?.toString().replace(/[^\d.-]/g, '')) || 0;
    ...
    if (existingItemIndex !== -1) {
        ...
        existingItem.hpp_total = existingItem.qty * existingItem.hpp_pcs; // ✅ Perbaikan
        ...
        const newItem = {
            ...
            cogs: product.cogs,              // ✅ TAMBAH: Field cogs
            quantity: product.quantity,
            price_pcs: price,                // ✅ Sekarang menggunakan cogs
            hpp_pcs: hpp,                    // ✅ HPP dari BOM
            qty: 1,
            subtotal: price,                 // ✅ Sekarang menggunakan cogs
            hpp_total: hpp                   // ✅ HPP total dari BOM
        };
...
function submitForm() {
    const form = $("#receipt-return-form");
    ...
    // PERBAIKAN: Pastikan semua field numeric tidak null
    cartState.items.forEach((item, index) => {
        formData.append(`products[${index}][id_product]`, item.id_product || 0);
        formData.append(`products[${index}][barcode]`, item.barcode || '');
        formData.append(`products[${index}][product_name]`, item.product_name || '');
        formData.append(`products[${index}][qty]`, item.qty || 0);
        formData.append(`products[${index}][quantity]`, item.quantity || 0);
        formData.append(`products[${index}][price_pack]`, parseFloat(item.cogs) || 0);
        formData.append(`products[${index}][hpp_pcs]`, parseFloat(item.hpp_pcs) || 0);
        formData.append(`products[${index}][hpp_total]`, parseFloat(item.hpp_total) || 0);
        formData.append(`products[${index}][subtotal]`, parseFloat(item.subtotal) || 0);
    });

    formData.append('hpp_amount', parseFloat(cartState.hpp) || 0);
    formData.append('bruto_amount', parseFloat(cartState.bruto) || 0);
    formData.append('discount_amount', parseFloat(cartState.discount1) || 0);
    formData.append('discount_amount2', parseFloat(cartState.discount2) || 0);
    formData.append('discount_amount3', parseFloat(cartState.discount3) || 0);
    formData.append('netto_amount', parseFloat(cartState.netto) || 0);
            ...
            if (response.success) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const printUrl = `/admin/inventory/receipt-return/${response.return_id}/print`;
                        window.open(printUrl, '_blank');

                        // Tunggu sebentar lalu redirect ke index
                        setTimeout(() => {
                            window.location.href = '/admin/inventory/receipt-return';
                        }, 500);
