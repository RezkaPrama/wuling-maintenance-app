...
function loadExistingShippingData() {
    ...
    $.ajax({
        url: `/api/v1/receipt-return/${shippingId}/details`,
        type: 'GET',
        headers: {
        ...
        success: function (response) {
            if (response.success) {
                const shipping = response.receipt_return;
                const details = response.receipt_return_details;
                ...
                if (details && details.length > 0) {
                    cartState.items = details.map(detail => ({
                        ...
                        cogs: parseFloat(detail.price_pack || 0),        // ✅ UBAH: gunakan cogs
...
function initializeFormValidation() {
    $("#receipt-return-edit-form").on('submit', validateFormSubmission);

    const form = document.getElementById('receipt-return-edit-form');
...
function addProductToCart(product) {
    // ✅ Gunakan COGS untuk harga transaksi
    const price = parseFloat(product.cogs?.toString().replace(/[^\d.-]/g, '')) || 0;

    // ✅ Gunakan HPP dari BOM (dari API yang sudah diupdate)
    ...
    if (existingItemIndex !== -1) {
        ...
        existingItem.hpp_total = existingItem.qty * existingItem.hpp_pcs; // ✅ Gunakan hpp_pcs yang sudah ada
        ...
        const newItem = {
            ...
            cogs: product.cogs || 0,           // ✅ TAMBAH field cogs
...
function submitForm() {
    ...
    const formData = new FormData($("#receipt-return-edit-form")[0]);
    ...
    // Send the request
    $.ajax({
        url: $("#receipt-return-edit-form").attr('action'),
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
