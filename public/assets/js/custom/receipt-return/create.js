...
function addProductToCart(product, stockInfo = null) {
    ...
    if (existingItemIndex !== -1) {
        ...
        const newItem = {
            ...
            subtotal: price,
            hpp_total: hpp,
            hpp_subtotal: hpp // ✅ PASTIKAN INI ADA
        };
...
function addCartRowOptimized(item) {
    ...
    const newRow = `
        <tr id="${rowId}">
            ...
            <td class="align-middle text-end">${formatMoney(item.hpp_pcs)}</td> <!-- ✅ TAMBAHKAN -->
            <td class="align-middle text-end hpp-subtotal-value">${formatMoney(item.hpp_subtotal)}</td> <!-- ✅ TAMBAHKAN -->
...
function addCartRow(item) {
    addCartRowOptimized(item, false);
}
...
function increaseItemQuantity(productId) {
    ...
    if (itemIndex !== -1) {
        ...
        item.hpp_subtotal = item.qty * item.hpp_pcs;
        updateCartRow(item);
        updateCartSummary();
    }
}
...
function submitForm() {
    ...
    cartState.items.forEach((item, index) => {
        ...
        formData.append(`products[${index}][hpp_subtotal]`, parseFloat(item.hpp_subtotal) || 0);
