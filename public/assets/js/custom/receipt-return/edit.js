            ...
            if (response.success) {
                ...
                if (details && details.length > 0) {
                    cartState.items = details.map(detail => ({
                        ...
                        hpp_subtotal: parseFloat(detail.hpp || 0) * parseInt(detail.quantity_pack || 1) // ✅ TAMBAHKAN INI
                    }));
    ...
    const newRow = `
        <tr id="${rowId}">
            ...
            <td>
                ...
                <input type="hidden" name="products[${item.id_product}][hpp_subtotal]" value="${item.hpp_subtotal}"> <!-- ✅ TAMBAHKAN -->
            </td>
...
function addProductToCart(product) {
    ...
    const hpp = parseFloat(product.hpp?.toString().replace(/[^\d.-]/g, '')) || 0; // ✅ GUNAKAN hpp dari API
    ...
    if (existingItemIndex !== -1) {
        ...
        existingItem.hpp_subtotal = existingItem.qty * hpp; // ✅ TAMBAHKAN INI
        ...
        const newItem = {
            ...
            subtotal: price,
            hpp_total: hpp,
            hpp_subtotal: hpp // ✅ TAMBAHKAN INI
        };
...
function appendProductToFormData(formData, item, index) {
    ...
    formData.append(`products[${index}][price_pack]`, item.price_sell); // ✅ FIX: gunakan price_sell
