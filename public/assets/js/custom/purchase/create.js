...
function initializeSelect2() {
    ...
    supplierSelect.on('change', function () {
        ...
        if (supplierId) {
            // Enable and load products for selected supplier
    ...
    productSelect.on('change', function () {
        ...
        if (selectedProduct && selectedProduct.id) {
            // Ambil data lengkap yang telah disimpan
            const fullProductData = $(this).find(':selected').data('full_product');
            if (fullProductData) {
                addToCart(fullProductData);
...
async function loadSupplierProducts(supplierId) {
    try {
        const response = await fetch(`/api/v1/products/search?supplier_id=${supplierId}`, {
            headers: {
        ...
        if (result.success) {
            ...
            result.data.forEach(product => {
                const option = new Option(
                    `${product.name} - ${product.type} (Stock: ${product.stock} ${product.id_uom})`,
                    product.id,
                    false,
                    false
                );

                // Store full product data in the option element
                $(option).data('full_product', {
                    id: product.id,
                    name: product.name,
                    type: product.type,
        ...
        showErrorAlert('Error loading products. Please try again.');
    }
}

function initializeEventListeners() {
    // Hapus event listener untuk searchInput karena kita menggunakan select2
    ...
    if (taxPercentageInput) {
        taxPercentageInput.addEventListener('input', handleTaxChange);
    }

    const discountPercentageInput = document.getElementById('discount_percentage');
    if (discountPercentageInput) {
        discountPercentageInput.addEventListener('input', handleDiscountChange);
    }

    const shippingAmountInput = document.getElementById('shipping_amount');
    if (shippingAmountInput) {
        shippingAmountInput.addEventListener('input', handleShippingChange);
    }

    // Get total amount button
    const getTotalAmountBtn = document.getElementById('getTotalAmount');
    if (getTotalAmountBtn) {
        getTotalAmountBtn.addEventListener('click', function () {
...
function addToCart(product) {
    ...
    // Jika product adalah hasil dari select2, data lengkap ada di full_product
    const productData = product.full_product || product;
    
    const existingItem = cartState.items.find(item => item.product_id === productData.id);
    const price = parseFloat(productData.price_buy) || 0; // Tambah fallback untuk price
    const stock = productData.stock !== null ? productData.stock : 0;
    const unit = productData.unit || productData.id_uom || 'pcs';
...
function updateCartQuantity(productId, quantity) {
    ...
    if (item) {
        const unit = item.unit || item.id_uom || 'pcs';
        const isKg = unit.toLowerCase() === 'kg';
        
        quantity = isKg ? parseFloat(quantity) : parseInt(quantity);
        
        if (isNaN(quantity) || quantity < 0.01) {
            quantity = 0.01;
        }
        
        // Batasi ke 2 digit desimal jika kg
        if (isKg) {
        ...
        if (row) {
            ...
            quantityInput.value = isKg ? quantity.toFixed(2) : quantity;
...
function createCartRow(item) {
    const unit = item.unit || item.id_uom || 'pcs';
    const isKg = unit.toLowerCase() === 'kg';
    const quantityStep = isKg ? '0.01' : '1'; // Ubah step jadi 0.01
    const currentQty = isKg ? item.quantity.toFixed(2) : item.quantity; // Ubah jadi 2 digit
    ...
    return `
        <tr data-product-id="${item.product_id}">
            ...
            <td class="align-middle text-center">
                <span class="badge badge-info">${item.stock} ${unit}</span>
            </td>
...
function attachCartEventListeners() {
    document.querySelectorAll('.quantity-input').forEach(input => {
        ...
        input.addEventListener('change', function(e) {
            let value = isKg ? parseFloat(this.value) : parseInt(this.value);
            ...
            // Format decimal untuk kg
            if (isKg) {
                this.value = value.toFixed(2); // Ubah jadi 2 digit
            }
        ...
        input.addEventListener('input', function(e) {
            ...
            if (!isNaN(value) && value > 0) {
                if (isKg) {
                    // Batasi input ke 2 digit desimal
