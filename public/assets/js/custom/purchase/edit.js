...
function initializeComponents() {
    ...
    // Add immediate product loading when editing
    const supplierSelect = $('#supplier_id');
    if (supplierSelect.val()) {
        const productSelect = $('#product-select');
        productSelect.prop('disabled', false);
        loadSupplierProducts(supplierSelect.val());
    }
}
    ...
    try {
        ...
        if (result.success) {
            ...
            result.data.forEach(product => {
                ...
                // Store full product data including unit information
...
function updateCartQuantity(productId, quantity) {
    ...
    if (item) {
        ...
        // Round to 2 decimal places if kg
        if (isKg) {
...
function createCartRow(item) {
    const unit = item.unit || item.id_uom || 'pcs';
    const isKg = unit.toLowerCase() === 'kg';
    const quantityStep = isKg ? '0.01' : '1';
    const currentQty = isKg ? item.quantity.toFixed(2) : item.quantity;
...
function attachCartEventListeners() {
    document.querySelectorAll('.quantity-input').forEach(input => {
        ...
        // Update input attributes based on unit
        if (isKg) {
        ...
        input.addEventListener('change', function(e) {
            ...
            if (isNaN(value) || value < (isKg ? 0.01 : 1)) {
                value = isKg ? 0.01 : 1;
                this.value = value;
            }
            
            // Format decimal places for kg
            if (isKg) {
        ...
        input.addEventListener('input', function(e) {
            ...
            // Allow decimal input for kg units
            if (isKg) {
                ...
                // Only allow integers for non-kg units
                if (/^\d+$/.test(value)) {
...
function validateForm() {
    ...
    for (const item of cartState.items) {
        const unit = item.unit || item.id_uom || 'pcs';
        const isKg = unit.toLowerCase() === 'kg';
        const minValue = isKg ? 0.01 : 1;
        
        if (isNaN(item.quantity) || item.quantity < minValue) {
            Swal.fire({
                icon: 'error',
                title: 'Validation Error',
                text: `Please enter a valid quantity for ${item.product_name} (minimum ${minValue} ${unit})`
            });
            return false;
        }
    }
