...
var PembayaranPiutang = (function () {
    ...
    function buildHistoryHtml(p) {
        ...
        p.history_pembayaran.forEach(function (h) {
            ...
            var jenisBadge = '';
            if (h.jenis === 'Retur') {
                // Tampilkan no_retur sebagai link jika tersedia
                var returLink = h.no_retur
                    ? ' <a href="/inventory/realisasi-retur-penjualan/' + escHtml(h.no_retur) + '"'
                    + '   target="_blank"'
                    + '   class="ms-1 text-danger fw-bold"'
                    + '   style="font-size:0.7rem"'
                    + '   title="Lihat Detail Retur">'
                    + '   <i class="bi bi-box-arrow-up-right"></i> ' + escHtml(h.no_retur)
                    + ' </a>'
                    : '';
