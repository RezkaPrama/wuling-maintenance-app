...
const cartState = {
    ...
    lastKeypressTime: 0,
    totalPack: 0,
    totalPasang: 0,  // ← TAMBAHKAN
    hppAmount: 0,
    scanMode: 'pack'  // ← TAMBAHKAN
};
            ...
            if (response.success) {
                ...
                if (details && details.length > 0) {
                    cartState.items = details.map(detail => ({
                        ...
                        quantity: parseInt(detail.qty || 12),
                        ...
                        hpp_total: parseFloat(detail.hpp || 0) *
                            (parseInt(detail.quantity_pack || 0) +
                                (parseInt(detail.quantity_pcs || 0) / parseInt(detail.qty || 12)))
                    }));
...
function updateCartSummary() {
    ...
    // Update displays
    $("#total-pack").text(cartState.totalPack);
    $("#total-pasang").text(cartState.totalPasang);  // ← TAMBAHKAN
    $("#total_pack").val(cartState.totalPack);
    $("#total_pasang").val(cartState.totalPasang);  // ← TAMBAHKAN
    $("#total-amount").text(formatMoney(cartState.bruto));
...
function addProductToCart(product) {
    ...
    if (existingItemIndex !== -1) {
        ...
        const newItem = {
            ...
            price_pcs: price,
            hpp_pcs: hpp,
            qty: cartState.scanMode === 'pack' ? 1 : 0,  // ← UPDATE
            qty_pcs: cartState.scanMode === 'pasang' ? 1 : 0,  // ← TAMBAHKAN
            subtotal: price,
            hpp_total: hpp
        };
...
function updateCartRow(item) {
    ...
    row.find('.product-qty-pcs').val(item.qty_pcs || 0);  // ← TAMBAHKAN
    row.find('.subtotal-value').text(`${formatMoney(item.subtotal)}`);

    // Update hidden inputs
    row.find(`input[name="products[${item.id_product}][qty]"]`).val(item.qty);
    row.find(`input[name="products[${item.id_product}][qty_pcs]"]`).val(item.qty_pcs || 0);  // ← TAMBAHKAN
    ...
    setTimeout(() => row.removeClass('updated-item-highlight'), 2000);
    setTimeout(() => $("#product-search").focus(), 100);
}
...
function submitForm() {
    ...
    const formData = new FormData($("#send-online-form")[0]);
    ...
    // Send the request
    $.ajax({
        url: $("#send-online-form").attr('action'),
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
function appendSummaryToFormData(formData) {
    ...
    formData.append('total_pasang', cartState.totalPasang);  // ← TAMBAHKAN
    ...
    if (response.success) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '/admin/manufacture/send-online';
            }
        });
    } else {
