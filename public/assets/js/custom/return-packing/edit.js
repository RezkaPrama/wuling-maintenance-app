...
function loadExistingShippingData() {
    ...
    $.ajax({
        url: `/api/v1/return-packing/${shippingId}/details`,
        type: 'GET',
        headers: {
        ...
        success: function (response) {
            if (response.success) {
                ...
                $('#type').val(shipping.type).trigger('change');
...
function initializeSelect2() {
    ...
    // TAMBAHKAN INISIALISASI UNTUK TYPE
    $('#type').select2({
        placeholder: 'Pilih Jenis Retur',
        allowClear: false,
        minimumResultsForSearch: Infinity // Hide search box karena hanya 2 pilihan
    });
...
function submitForm() {
    ...
    const formData = new FormData($("#return-packing-form")[0]);
    ...
    // Send the request
    $.ajax({
        url: $("#return-packing-form").attr('action'),
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
